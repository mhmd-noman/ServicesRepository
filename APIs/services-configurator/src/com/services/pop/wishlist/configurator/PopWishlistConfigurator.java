package com.services.pop.wishlist.configurator;
...
public class PopWishlistConfigurator {
	private static final Logger logger = LoggerFactory.getLogger(PopWishlistConfigurator.class);

	public MainResponseObject popWishlist(MainRequestObject mainRequestObject, Connection con) throws BaseException {
	...
	private void mapRequest(MainRequestObject mainRequestObject, CustomerServicesRequest customerServicesRequest) {
		if (null != mainRequestObject) {
			logger.info(logger.isInfoEnabled() ? "Requested content recieved for popWishlist: [" +mainRequestObject+ "]": null);
			customerServicesRequest.setWishlist(mainRequestObject.getWislist());
		}
		customerServicesRequest.setCustomerServicesAction(CustomerServicesAction.POP_WISHLIST);
		mainRequestObject.setServiceId(CustomerServicesAction.POP_WISHLIST.toString());
	}
	...
	private void mapResponse(MainResponseObject mainResponseObject, CustomerServicesResponse customerServicesResponse) throws BaseException {
		logger.info(logger.isInfoEnabled() ? "Response recieved for popWishlist: [" +customerServicesResponse+ "]": null);
