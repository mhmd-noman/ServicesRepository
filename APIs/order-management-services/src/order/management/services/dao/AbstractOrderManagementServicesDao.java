...
public abstract class AbstractOrderManagementServicesDao {
	...
	public static final String CANCEL_ORDER            = "update orders set order_status = 'F', cancelled_at = ? where order_id = ?";
	public static final String GET_ORDERS              = "select o.order_id, o.order_description, o.cust_name, o.cust_phone, o.cust_phone2, o.cust_address, o.cust_address2, o.cust_email, o.area, o.city, o.state, o.country, o.order_org_amount, o.order_rtl_amount, o.order_calc_discount, o.created_on, o.cancelled_at, o.order_status, op.product_quantity, op.product_org_price, op.product_rtl_price, op.product_discount, op.product_net_price, p.id, p.name, p.company, p.category, p.flavour, p.quantity, p.weight, p.servings, p.serving_size, p.org_price, p.discount, p.mfg_date, p.expiry_date, p.bar_code, p.direction_to_use, p.description, p.created_on, p.last_updated_on, p.image1, p.image2, p.image3, p.image4, p.image5, p.is_active from orders o, order_products op, products p where o.order_id = op.order_id and op.product_id = p.id ";
	public static final String SUBT_PRODUCTS_QUANTITY  = "update products set quantity = (quantity - ?) where id = ? ";
	public static final String ADD_PRODUCTS_QUANTITY   = "update products set quantity = (quantity + ?) where id = ? ";
